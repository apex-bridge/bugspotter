...
function useStableKeys(length: number): {
  ...
  //
  // Subtle: we must track the *previous* `length` prop (not
  // `keys.length`) and update the ref on every render. An earlier
  // version compared against `keys.length`, but after an internal
  // `insertAt` (`keys` grows first, then parent re-renders with the
  // larger `length`) the prop and `keys.length` are already equal —
  // the if-block is skipped and the ref never advances. The next
  // *external* length change that happens to match the stale ref
  // value would then be missed and the arrays drift permanently.
  const lastLengthRef = useRef(length);
  const prevLength = lastLengthRef.current;
  lastLengthRef.current = length;
  if (length !== prevLength && keys.length === prevLength) {
    setKeys((prev) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        {/*
          Key by rule identity so switching between two rules with the
          same number of conditions/actions still remounts sub-editors.
          `useStableKeys` deliberately preserves slot identity across
          equal lengths — without a fresh remount here, `WebhookFields`
          (and any future sub-editor with prop-seeded internal state)
          would carry stale text from the previously-edited rule.
        */}
        <fieldset key={editingRule?.id ?? 'new'} disabled={readOnly} className="space-y-6">
          {/* Name */}
