...
export default function VerifyEmailPage() {
  ...
  const [status, setStatus] = useState<Status>(() => {
    ...
    return hasTokenParam ? 'invalid' : 'no_token';
  });
  ...
  // Strip `?token=` so the link can't be replayed from browser
  // history. Run for any present `?token=` param — including the
  // empty-string case — so the address bar doesn't hold a stray
  // `?token=` after the page renders the invalid state. We use
  // `setSearchParams` (rather than `window.history.replaceState`)
