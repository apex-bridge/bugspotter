...
- **Flywheel** - **partial** (main gap). The left half (collection) works: immutable `intelligence_event`
  records and accumulating `intelligence_feedback` verdicts (ADR-0032), bug lifecycle signal. The right half (returning feedback
...
- The main gap is localised: close the flywheel (feedback -> quality) and introduce an eval-gate.
  The remaining open items (monitoring/drift alerts - R3, estimate-vs-actual - R4) are real technical
  roadmap work; the only gaps that are purely descriptive are the framework-label overlays.
