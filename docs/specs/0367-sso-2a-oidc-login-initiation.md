...
New file. Route module, not a Fastify plugin — registered by a direct function call from `server.ts`, matching this backend's general convention of wiring route modules that way rather than via `fastify.register(...)` (reserved for actual Fastify plugins). The exact signature varies per module by what dependencies it needs — `authRoutes(fastify, db)`, `userRoutes(fastify, userRepo)`, `deploymentRoutes(fastify)` with no second argument at all — so `oidcRoutes(fastify)` (no second argument, matching `deploymentRoutes`'s shape) is consistent with that variety, not a single uniform signature every route module shares. Only the `login` handler in this slice — #368 appends the `callback` handler to this same file.
