...
- Retry scheduling. `retryAfter` is attached to the error as metadata only: **no caller reads it in this change**, and neither `isRetryableError` nor `calculateBackoff` is modified. This deliberately narrows issue #269's "honor the `Retry-After` header value for backoff timing" bullet. The reason is that `IntelligenceClient` is shared by callers with opposite constraints, and `requestWithRetry` runs in all of them: `src/queue/workers/intelligence-worker.ts` can absorb a 30 s to 120 s wait, while `src/api/routes/sdk-similar.ts`, `intelligence.ts`, `self-service.ts` and `admin-intelligence.ts` cannot. A sleep inside the shared client would therefore land in an SDK-facing request handler. The wait belongs to whichever caller can absorb it, not to the client, so honouring the header is deferred to a follow-up scoped to the worker's job rescheduling. The behavioural half of this spec is the circuit-breaker exclusion, not the delay hint.
