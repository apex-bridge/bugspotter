...
- **[#97](https://github.com/apex-bridge/bugspotter/issues/97) — Audit-identity masking** (partial fix; #97 stays open with narrowed scope): handler-level audit logs and the global audit middleware now record `userId: authUser?.id ?? null` and `apiKeyId: apiKey?.id ?? null` as separate fields (Option B from the issue). `user_id` lands in its dedicated column; `api_key_id` lands in the `details` JSONB until the schema-level column ([#104](https://github.com/apex-bridge/bugspotter/issues/104)) lands. ESLint rule `no-restricted-syntax` (root config) prevents regression on the `'api-key'` literal pattern. **Still open**: the dual-header (JWT + api-key) case still records `user_id: null` because the auth middleware short-circuits on api-key and never populates `request.authUser`, so the JWT user's id is unreachable from the audit hook. PR-105 fixes the literal-masking and adds first-class machine attribution; closing the human-attribution gap requires a deeper auth-middleware change (run JWT parsing alongside api-key validation for identity purposes — separate from the precedence decision) and is what keeps #97 open.
