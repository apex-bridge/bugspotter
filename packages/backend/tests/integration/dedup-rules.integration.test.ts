...
describe('Migration 025 — dedup_rules cascade trigger + seed', () => {
  ...
  describe('AFTER DELETE trigger on dedup_rules', () => {
    ...
    it('does not affect throttle rows for unrelated rule IDs', async () => {
      ...
      // clause filters by rule_id, not by table. Scope the assertion
      // to OUR two forged rule_ids; the testcontainer DB is shared
      // across all integration tests, so other suites' throttle rows
