...
describe('linearPlugin', () => {
  ...
  describe('validateConfig', () => {
    ...
    it('fails when teamKey does not match the canonical key of the resolved team', async () => {
      const service = linearPlugin.factory(mockContext);
      const result = await service.validateConfig({
        apiKey: 'lin_api_xxx',
        teamId: 'team-uuid',
        teamKey: 'BOGUS', // resolved team's key is ENG (per shared mock)
      });
      expect(result.valid).toBe(false);
      expect(result.error?.toLowerCase()).toContain('mismatch');
      expect(result.details?.canonicalTeamKey).toBe('ENG');
    });
  });
});
