...
Check for:
0. A method called on a type imported from another package (\`import type { X } from '@bugspotter/other-pkg'\`) where that package's defining file is NOT among the source files above. You cannot confirm or deny the method exists without that file - flag this explicitly as "unverified: <package>'s defining file for <type> was not available to check" rather than assuming it's correct because nothing else looks wrong. This is not the same failure as item 1 below: item 1 is a check you can actually perform; this is a check you structurally cannot perform yet, and saying so is the correct output, not a cop-out.
